...
def train():
    ...
    for epoch in range(config['num_epochs']):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            graph, constituent_starts, constituent_ends, constituent_labels \
                = [b.to(device) for b in batch[1:]]
            bert_optimizer.zero_grad()
            task_optimizer.zero_grad()
            _, loss = model(input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids,
...
def evaluate(model, eval_dataloader, data_path, singleton, device):
    ...
    with torch.no_grad():
        for i, (batch, example) in enumerate(zip(eval_dataloader, examples)):
            ...
            input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids, sentence_map, \
            subtoken_map, graph, constituent_starts, constituent_ends, constituent_labels \
                = [b.to(device) for b in batch[1:]]
            predictions, loss = model(input_ids, input_mask, text_len, genre, gold_starts, gold_ends,
