...
class CoNLLCorefResolution(object):
    def __init__(self, doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids,
                 sentence_map, subtoken_map, flatten_sentences, sliding_windows, sentence_masks, token_start_ids,
                 token_end_ids):
...
class CoNLLDataset(Dataset):
    ...
    def create_graph(self, doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends,
        ...
        # forward edge type: 0 1  backward edge type: 2 3 self-loop edge type: 4
        # cons-token start: 5  cons-token end: 6
        # ablation study:
        # 1. no dual graph -> parent-child edge: 0  grandparent-grandchild edge: 1  self-loop edge: 2  cons-token 3
        # 2. no multi-type edges -> parent-child edge: forward 0 backward 1  self-loop edge: 2  cons-token 3
        # 3. no multi-type edges + dual graph -> parent-child edge: 0  self-loop edge: 1  cons-token: 2
        ...
        for high_order_sent_cons in constituent:
            for i, sent_cons in enumerate(high_order_sent_cons):
                for idx, start, end, label, parent_idx in sent_cons:
                    idx_nodeid = idx - constituent_start_idx + node_id_offset
                    if parent_idx == -1:
        ...
        return (doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids, sentence_map,
...
def tensorize_example(example: dict, config: dict, tokenizer: BertTokenizer, genres: dict) -> CoNLLCorefResolution:
    ...
    return CoNLLCorefResolution(doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids,
                                sentence_map, subtoken_map, flatten_sentences, sliding_windows, sentence_masks,
                                token_start_ids, token_end_ids)


def convert_to_sliding_window(doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids,
                              sentence_map, subtoken_map, token_start_ids, token_end_ids, flatten_sentences,
                              sentence_masks, sliding_windows, sliding_window_size):
...
def truncate_example(doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids, sentence_map,
                     subtoken_map, token_start_ids, token_end_ids, config):
    ...
    return (doc_key, input_ids, input_mask, text_len, genre, gold_starts, gold_ends, cluster_ids, sentence_map,
            subtoken_map, token_start_ids, token_end_ids)
