...
class CorefModel(nn.Module, ABC):
    def __init__(self, config, cons_tag2id):
        ...
        self.ct_gat_layer = MultiHeadGATLayer(CTGATLayer, config['hidden_size'] * 2 + config['cons_hidden_size'],
                                              ...
                                              config)
        self.multi_order_linear_attn_layer = self.ffnn(
    ...
    def forward(self, input_ids, input_mask, text_len, speaker_ids, genre, gold_starts, gold_ends, cluster_ids,
        ...
        """
            ...
            Args:
                ...
                sentence_ids: [num_sentences, max_seq_len]
                sentence_mask: [num_sentences, max_seq_len]
        ...
        for i in range(self.config['num_iterations']):
            ...
            attn_weights = torch.softmax(self.multi_order_linear_attn_layer(constituent_embeddings).squeeze(2), dim=1)
        ...
        # # constituent trees as mention filter
        # constituent_starts_cpu = constituent_starts.cpu().tolist()
        # constituent_ends_cpu = constituent_ends.cpu().tolist()
        # constituent_labels_cpu = constituent_labels.cpu().tolist()
        # constituent_spans = {(start, end): label for start, end, label in
        #                      zip(constituent_starts_cpu, constituent_ends_cpu, constituent_labels_cpu)}
        # valid_constituents = ['VV', 'NT', 'DFL', 'PN', 'NR', 'NP', 'QP', 'NN']
        # valid_constituents = [self.cons_tag2id[constituent] for constituent in valid_constituents]
        # candidate_starts_cpu = candidate_starts.cpu().tolist()
        # candidate_ends_cpu = candidate_ends.cpu().tolist()
        # top_span_indices = []
        # for i, (start, end) in enumerate(zip(candidate_starts_cpu, candidate_ends_cpu)):
        #     if (start, end) in constituent_spans and constituent_spans[(start, end)] in valid_constituents:
        #         top_span_indices.append(i)
        # top_span_indices = torch.tensor(top_span_indices, dtype=torch.int64, device=candidate_mention_scores.device)
        # top_span_mention_scores = candidate_mention_scores[top_span_indices]
        # top_span_starts = candidate_starts[top_span_indices]
        # top_span_ends = candidate_ends[top_span_indices]
        # top_span_emb = candidate_span_emb[top_span_indices]
        # top_span_cluster_ids = candidate_cluster_ids[top_span_indices]
        # k = top_span_indices.size()[0]
        # c = min(self.config["max_top_antecedents"], k)
        ...
        if self.config['mention_loss']:
            ...
            loss += self.config['mention_loss_ratio'] * proposal_loss.mean()
