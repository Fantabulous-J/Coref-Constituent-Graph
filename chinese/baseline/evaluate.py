...
def evaluate(model, eval_dataloader, data_path, conll_path, prediction_path, device):
    ...
    with torch.no_grad():
        for i, (batch, example) in enumerate(zip(eval_dataloader, examples)):
            ...
            # predict_labels = torch.sigmoid(top_span_mention_scores)
            # mention_tp += torch.logical_and(predict_labels > 0.5, top_span_cluster_ids > 0).sum()
            # mention_fp += torch.logical_and(predict_labels > 0.5, top_span_cluster_ids == 0).sum()
            # mention_fn += torch.logical_and(predict_labels <= 0.5, top_span_cluster_ids > 0).sum()
            if i % 10 == 0:
    ...
    return coref_p, coref_r, coref_f, conll_results, analysis_results
...
if __name__ == '__main__':
    ...
    model = CorefModel(config, dataloader.tokenizer)
    # util.load_from_pretrained_coref_tf_checkpoint(model, config['tf_checkpoint'])
    ...
    # eval on dev set
    coref_p, coref_r, coref_f, dev_conll_results, analysis_results = evaluate(model, eval_dataloader,
                                                                              config['eval_path'],
    ...
    test_coref_p, test_coref_r, test_coref_f, test_conll_results, test_analysis_results = \
